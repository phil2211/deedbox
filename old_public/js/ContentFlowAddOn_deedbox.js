/*  ContentFlowAddOn_deedbox, version 1.0.2 
 ...
 * 2. Change the string 'deedbox' in the 'new ContentFlowAddOn' line to 'ADDONNAME'
...
new ContentFlowAddOn ('deedbox', {
     ...
     */
	ContentFlowConf: {
        loadingTimeout: 100000,         // milliseconds
        ...
        startItem:  "first item",           // which item should be shown on startup?
        scrollInFrom: "visiblePre",            // from where should be scrolled in?
        ...
        reflectionColor: "none", // none, transparent, overlay or hex RGB CSS style #RRGGBB
        ...
        onMakeActive: function (item) {
            if ($("#documentForm").val() != null) { 
                $("#documentForm").load('/ajax/editform/document/' + item.item.id);
                $("#documentForm").ajaxComplete(function() {
                    $("#confirm").click(function() {
                        $("#documentForm").triggerHandler("change");
                        return false;
                    });
                });
            }
            
            if ($("#docstats").val() != null) {
                $("#docstats").attr('href', '/search/stats/document/' + item.item.id);
            }
            
            if ($("#editdoc").val() != null) {
                $("#editdoc").attr('href', '/edit/showbig/documentId/' + item.item.id);
            }
        },
